  ...
  return (
    <AppShell>
      ...
      <div className="flex items-center justify-between px-4 md:px-6 h-14 shrink-0 gap-2"
        style={{ borderBottom: '1px solid var(--border)', background: 'var(--bg-card)', backdropFilter: 'blur(24px)' }}>
        <div>
          <h1 className="text-[15px] md:text-[16px] font-bold" style={{ color: 'var(--text-primary)' }}>Segmentler</h1>
      ...
      <div className="flex flex-col lg:flex-row flex-1 min-h-0 lg:overflow-hidden">
            ...
            {[
              ...
              <div key={kpi.label} className="rounded-2xl p-4 relative overflow-hidden min-w-0"
                ...
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'var(--text-secondary)' }}>{kpi.label}</p>
                ...
                <p className={cn('font-bold mb-1 leading-tight', (kpi as { small?: boolean }).small ? 'text-[13px]' : 'text-[20px]')}
                  style={{ color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>{kpi.value}</p>
                    ...
                    <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                      {[180, 60, 80, 200, 50, 80].map((w, j) => (
                        <td key={j} className="px-4 py-4">
                          <div className="h-4 rounded-md animate-pulse" style={{ background: 'var(--bg-hover)', width: w }} />
                        </td>
                      ))}
                    </tr>
                  ))
                        ...
                        <div className="relative">
                          ...
                          {openMenuId === seg.id && (
                            <div className="absolute right-0 top-full mt-1 w-36 rounded-xl z-20 overflow-hidden"
                              style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
                              {seg.type !== 'builtin' && (
                                <button
        ...
        {showCreate ? (
          ...
          <div className="w-full lg:w-80 shrink-0 flex flex-col overflow-y-auto"
            style={{ borderTop: '1px solid var(--border)', borderLeft: '1px solid var(--border)', background: 'var(--bg-card)' }}>
